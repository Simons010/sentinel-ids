                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="p-2 hover:bg-[#22D3EE]/10 focus-visible:bg-[#22D3EE]/10 rounded transition-colors group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#22D3EE]"
                        title="View logs"
                        aria-label={`View logs for ${item.filename}`}
                      >
                        <Eye className="w-4 h-4 text-gray-400 group-hover:text-[#22D3EE] group-focus-visible:text-[#22D3EE]" />
                      </button>
                      {item.status !== "processing" && (
                        <button
                          onClick={() => onReanalyze?.(item)}
                          disabled={actionsDisabled}
                          className="p-2 hover:bg-[#10B981]/10 focus-visible:bg-[#10B981]/10 rounded transition-colors group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#10B981]"
                          title="Reanalyze"
                          aria-label={`Reanalyze ${item.filename}`}
                        >
                          <RotateCw className="w-4 h-4 text-gray-400 group-hover:text-[#10B981] group-focus-visible:text-[#10B981]" />
                        </button>
                      )}
                      <button
                        onClick={() => setDeleteTarget(item)}
                        disabled={actionsDisabled}
                        className="p-2 hover:bg-[#EF4444]/10 focus-visible:bg-[#EF4444]/10 rounded transition-colors group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4444]"
                        title="Delete"
                        aria-label={`Delete ${item.filename}`}
                      >
                        <Trash2 className="w-4 h-4 text-gray-400 group-hover:text-[#EF4444] group-focus-visible:text-[#EF4444]" />
                      </button>
                    </div>
                  </td>
                </tr>

...
## Usage
- **Log Ingestion**: Send POST requests to `/api/logs/`.
- **Anomalies**: View real-time alerts on the Dashboard.
- **ML Training**: The model is trained on startup or can be retrained using `python ml_engine/models/train_initial.py`.

## API Documentation
- `POST /api/logs/`: Ingest log data (JSON).

...
class NetworkStatsView(APIView):
    def get(self, request):
        ...
        # ⚡ Bolt: Optimized N+1 queries. Replaced 168 individual count() queries
        # with a single aggregated query using TruncDate and ExtractHour.
        heatmap = []

        aggregated_data = alerts.annotate(
            date=TruncDate('created_at'),
            hour=ExtractHour('created_at')
        ).values('date', 'hour').annotate(count=Count('id'))

        heatmap_dict = {(d['date'], d['hour']): d['count'] for d in aggregated_data if d['date']}

        for days_offset in range(7):
            day = (timezone.now() - timedelta(days=6 - days_offset)).date()
            for hour in range(24):
                count = heatmap_dict.get((day, hour), 0)

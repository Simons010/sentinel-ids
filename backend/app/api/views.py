...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        # ⚡ Bolt: Optimize N+1 query loop by using a single GROUP BY query
        severity_counts = {s: 0 for s in ["critical", "high", "medium", "low"]}
        for r in alerts_24h.values('severity').annotate(count=Count('severity')).order_by():
    ...
    def get(self, request):
        ...
        for r in alerts_24h.values('severity').annotate(count=Count('severity')).order_by():
            if r['severity'] in severity_breakdown:
                severity_breakdown[r['severity']] = r['count']

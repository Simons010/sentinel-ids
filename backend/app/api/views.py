...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        # Fetch data in memory to avoid N+1 queries (72 queries -> 2 queries)
        ...
        for i in range (24):
            ...
            normal = sum(1 for log in logs_data if hour_start <= log['timestamp'] < hour_end and not log['is_suspicious'])
            suspicious = sum(1 for log in logs_data if hour_start <= log['timestamp'] < hour_end and log['is_suspicious'])
            confirmed = sum(1 for alert in alerts_data if hour_start <= alert['created_at'] < hour_end)
...
class AnalyticsView(APIView):
    def get(self, request):
        ...
        # Fetch data in memory to avoid N+1 queries (72 queries -> 2 queries)
        # Note AnalyticsView uses created_at for logs instead of timestamp
        ...
        for i in range(24):
            ...
            normal = sum(1 for log in logs_data if hour_start <= log['created_at'] < hour_end and not log['is_suspicious'])
            suspicious = sum(1 for log in logs_data if hour_start <= log['created_at'] < hour_end and log['is_suspicious'])
            confirmed = sum(1 for alert in alerts_data if hour_start <= alert['created_at'] < hour_end)

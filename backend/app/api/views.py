...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        # ⚡ Bolt: Optimize severity queries from N+1 filter().count() loops to a single annotated query
        severity_counts = {s: 0 for s in ["critical", "high", "medium", "low"]}
        for agg in alerts_24h.values('severity').annotate(count=Count('id')).order_by():

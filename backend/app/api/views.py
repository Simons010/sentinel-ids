...
def _analyze_uploaded_file(upload_id, force=False):
    ...
    upload.save(
        update_fields=["status", "error_message", "threats_found", "clean_logs"]
    )
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        for i in range(24):
            ...
            normal = logs_24h.filter(
                timestamp__gte=hour_start, timestamp__lt=hour_end, is_suspicious=False
            ).count()
            suspicious = logs_24h.filter(
                timestamp__gte=hour_start, timestamp__lt=hour_end, is_suspicious=True
            ).count()
        ...
        top_sources = list(
            ...
            .values("log__src_ip")
            .annotate(count=Count("id"), max_score=Max("severity_score"))
            .order_by("-count")[:5]
        )
        ...
        threshold = 0.5
        all_annotated = NetworkLog.objects.annotate(
            effective_score=Greatest("ai_score", "ml_score")
        )

        tp = all_annotated.filter(
            is_suspicious=True, effective_score__gte=threshold
        ).count()
        tn = all_annotated.filter(
            is_suspicious=False, effective_score__lt=threshold
        ).count()
        fp = all_annotated.filter(
            is_suspicious=False, effective_score__gte=threshold
        ).count()
        fn = all_annotated.filter(
            is_suspicious=True, effective_score__lt=threshold
        ).count()
        ...
        return Response(
            {
                ...
                "anomaly_detection_rate": round(
                    logs_24h.filter(is_suspicious=True).count()
                    / (logs_24h.count() or 1)
                    * 100,
                    1,
                ),
    ...
    def _get_ai_summary(self, alerts_qs):
        ...
        if latest.log:
            confidence = (
                latest.log.ai_score if latest.log.ai_score > 0 else latest.log.ml_score
            )
        ...
        return {
            ...
            "severity_score": (
                round(latest.severity_score * 25, 1) if latest.severity_score else 0
            ),
    ...
    def get(self, request):
        ...
        if latest:
            ...
            if latest.log:
                confidence = (
                    latest.log.ai_score
                    if latest.log.ai_score > 0
                    else latest.log.ml_score
                )
...
class NetworkStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Stat cards
        unique_ips = (
            NetworkLog.objects.exclude(src_ip=None).values("src_ip").distinct().count()
        )
        ...
        for days_offset in range(7):
            ...
            heatmap.append(
                {"day": day.strftime("%a"), "date": day_str, "hours": day_row}
            )
        # Heatmap summary stats
        all_hourly = [h for row in heatmap for h in row["hours"]]
        peak_hour_attacks = max(all_hourly) if all_hourly else 0
        total_weekly = sum(all_hourly)

        # Find most active hour across all days
        hour_totals = [sum(row["hours"][h] for row in heatmap) for h in range(24)]
        ...
        critical_sources = sum(1 for s in geo_sources if s["risk"] >= 4)
        high_sources = sum(1 for s in geo_sources if s["risk"] == 3)
...
class LogUploadView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            upload.save()
            return Response(
                {"error": "Failed to process file", "details": str(e)}, status=500
            )
...
class LogUploadPreviewView(APIView):
    ...
    def get(self, request, upload_id):
        ...
        try:
            ...
            return Response(
                {"error": "Failed to read upload", "details": str(e)}, status=500
            )
...
class LogUploadAnalyzeView(APIView):
    ...
    def post(self, request, upload_id):
        ...
        force = (
            bool(request.data.get("force")) if isinstance(request.data, dict) else False
        )
    ...
    def post(self, request, upload_id):
        ...
        with analysis_queue_lock:
            if upload_id in analysis_queue_state:
                ...
                filtered = [
                    (uid, frc) for uid, frc in analysis_queue if uid != upload_id
                ]
...
class AnalyticsView(APIView):
    def get(self, request):
        ...
        tp = annotated_logs.filter(
            is_suspicious=True, effective_score__gte=threshold
        ).count()
        tn = annotated_logs.filter(
            is_suspicious=False, effective_score__lt=threshold
        ).count()
        fp = annotated_logs.filter(
            is_suspicious=False, effective_score__gte=threshold
        ).count()
        fn = annotated_logs.filter(
            is_suspicious=True, effective_score__lt=threshold
        ).count()
        ...
        for i in range(24):
            ...
            normal = NetworkLog.objects.filter(
                created_at__gte=hour_start, created_at__lt=hour_end, is_suspicious=False
            ).count()
            suspicious = NetworkLog.objects.filter(
                created_at__gte=hour_start, created_at__lt=hour_end, is_suspicious=True
            ).count()
        ...
        suspicious_logs = logs.filter(
            Q(is_suspicious=True) | Q(ai_score__gt=0) | Q(ml_score__gt=0)
        )

        # Use the best available score for each log (AI score takes precedence if it was just added)
        dist_data = (
            suspicious_logs.annotate(
                effective_score=Greatest("ai_score", "ml_score"),
                bucket=Case(
        ...
        avg_confidence = (
            suspicious_logs.annotate(
                effective_score=Greatest("ai_score", "ml_score")
            ).aggregate(avg=Avg("effective_score"))["avg"]
            or 0
        )
...
class ReportView(APIView):
    def post(self, request):
        ...
        end_at = timezone.make_aware(
            datetime(end_dt.year, end_dt.month, end_dt.day, 23, 59, 59, 999999)
        )

...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        logs_aggregated = logs_24h.aggregate(**log_aggs)
        alerts_aggregated = alerts_24h.aggregate(**alert_aggs)

        hourly_data = []
        for i in range(24):
            hourly_data.append({
                "hour": hour_starts[i].strftime("%H:%M"),
                "normal": logs_aggregated.get(f"normal_{i}", 0),
                "suspicious": logs_aggregated.get(f"suspicious_{i}", 0),
                "confirmed": alerts_aggregated.get(f"confirmed_{i}", 0)
            })
...
class AnalyticsView(APIView):
    def get(self, request):
        ...
        logs_aggregated = NetworkLog.objects.filter(created_at__gte=last_24h).aggregate(**log_aggs)
        alerts_aggregated = Alert.objects.filter(created_at__gte=last_24h).aggregate(**alert_aggs)

...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        log_results = logs_24h.aggregate(**log_aggregations)
        alert_results = alerts_24h.aggregate(**alert_aggregations)

        for i in range(24):
            hourly_data.append({
                "hour": hour_starts[i].strftime("%H:%M"),
                "normal": log_results.get(f'normal_{i}', 0) or 0,
                "suspicious": log_results.get(f'suspicious_{i}', 0) or 0,
                "confirmed": alert_results.get(f'confirmed_{i}', 0) or 0
            })

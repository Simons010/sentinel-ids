...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        for i in range(24):
            ...
            time_buckets.append((hour_start, hour_end))
        ...
        hourly_data = []
        for i, (hour_start, _) in enumerate(time_buckets):
            hourly_data.append({
...
class AnalyticsView(APIView):
    def get(self, request):
        ...
        log_results = NetworkLog.objects.filter(created_at__gte=last_24h).aggregate(**log_aggs)
        alert_results = Alert.objects.filter(created_at__gte=last_24h).aggregate(**alert_aggs)

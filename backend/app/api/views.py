...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        # Optimize N+1 queries for severity frequency distribution
        severity_counts = {s: 0 for s in ["critical", "high", "medium", "low"]}
        for entry in alerts_24h.values('severity').annotate(count=Count('severity')).order_by():
...
class ThreatsStatsView(APIView):
    def get(self, request):
        ...
        for entry in alerts_24h.values('severity').annotate(count=Count('severity')).order_by():
            if entry['severity'] in severity_breakdown:

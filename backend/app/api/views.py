...
class DashboardStatsView(APIView): 
    ...
    def get(self, request):
        ...
        # ⚡ Bolt: Replaced iterative .filter().count() with a single .values().annotate() query
        # Impact: Eliminates N+1 queries when calculating severity breakdown, improving dashboard load time
        severity_counts = {s: 0 for s in ["critical", "high", "medium", "low"]}
        severity_aggregates = alerts_24h.values('severity').annotate(count=Count('id')).order_by()
        for agg in severity_aggregates:
            if agg['severity'] in severity_counts:
...
class ThreatsStatsView(APIView):
    def get(self, request):
        ...
        # ⚡ Bolt: Replaced iterative .filter().count() with a single .values().annotate() query
        # Impact: Eliminates N+1 queries when calculating severity breakdown, improving threats page load time
        severity_breakdown = {s: 0 for s in ["critical", "high", "medium", "low", "informational"]}
        severity_aggregates = alerts_24h.values('severity').annotate(count=Count('id')).order_by()
        for agg in severity_aggregates:
            if agg['severity'] in severity_breakdown:
